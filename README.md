...
- 已将工作流升级为显式有向无环状态图：数据采集完成后并行执行业绩、风险和同类比较分支，再汇聚到因素讨论、合规审核和回答生成节点。
- 状态图启动时会校验重复节点、缺失依赖和循环依赖；任务恢复根据成功节点集合继续调度，阶段重跑只清理目标节点及其图后继节点。
...
- 已基于现有净值序列补充区间年化收益、下行波动率、收益回撤比、样本数量和观察跨度，并在报告中明确短样本边界。
- 核心周期收益严格按有效样本计算：近 1 月、3 月、6 月、1 年分别至少需要 23、67、133、253 条净值；样本不足时接口返回空值，前端显示 `--`，不再用较短区间冒充完整周期。
- 基金分析接口会返回样本数量、样本起止日期、观察天数和样本边界说明；图表仍只展示最近 120 条净值，指标计算使用最多 320 条净值。
...
默认情况下，Agent 使用本地确定性分析，不依赖真实 LLM Key。需要启用 AgentScope + OpenAI 兼容模型时，在项目根目录创建 `config/application-private.yml`：
...
`application.yml` 会通过 `optional:file:` 自动加载该文件，`config/application-private.yml` 已加入 Git 忽略规则。真实 URL 和密钥不要写入任何受 Git 跟踪的配置。启用后，各分析阶段会调用 AgentScope；调用失败时自动降级到本地确定性分析。

前端思考模式与 OpenAI `reasoning_effort` 的映射：
